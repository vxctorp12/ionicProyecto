<template>
  <ion-header>
    <ion-toolbar>
      <ion-buttons slot="start">
        <ion-button color="medium" @click="cancel">Cancelar</ion-button>
      </ion-buttons>
      <ion-title>{{ grado ? 'Editar Grado' : 'Nuevo Grado' }}</ion-title>
      <ion-buttons slot="end">
        <ion-button :strong="true" color="primary" @click="confirm">Guardar</ion-button>
      </ion-buttons>
    </ion-toolbar>
  </ion-header>

  <ion-content class="ion-padding">
    <div class="input-group">
      <label>Nombre del Grado</label>
      <div class="input-wrapper">
        <ion-input 
            v-model="nombre" 
            placeholder="Ej. Noveno Grado" 
            class="custom-input"
        ></ion-input>
      </div>
    </div>
  </ion-content>
</template>

<script setup lang="ts">
import { ref, onMounted } from 'vue';
import { modalController } from '@ionic/vue';
import { 
  IonHeader, IonToolbar, IonButtons, IonButton, IonTitle, IonContent, IonInput 
} from '@ionic/vue';

const props = defineProps<{ grado?: any; }>();
const nombre = ref('');

onMounted(() => {
  if (props.grado) {
    nombre.value = props.grado.nombre;
  }
});

const cancel = () => modalController.dismiss(null, 'cancel');

const confirm = () => {
  if (!nombre.value) {
    alert('El nombre es obligatorio');
    return;
  }
  modalController.dismiss({ nombre: nombre.value }, 'confirm');
};
</script>

<style scoped>
.input-group { margin-top: 20px; }
.input-group label { display: block; font-size: 13px; font-weight: 600; color: #333; margin-bottom: 8px; }
.input-wrapper { border: 1px solid #E0E0E0; border-radius: 8px; background: #FFFFFF; padding: 0 12px; transition: all 0.3s; }
.input-wrapper:focus-within { border-color: var(--ion-color-primary); box-shadow: 0 0 0 4px rgba(var(--ion-color-primary-rgb), 0.15); }
.custom-input { --padding-start: 0; --background: transparent; --placeholder-color: #A0A0A0; --color: #1A1A1A; height: 45px; font-size: 14px; --highlight-height: 0; }
</style>
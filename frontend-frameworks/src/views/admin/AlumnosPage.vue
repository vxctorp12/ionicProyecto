<template>
  <v-container>
      ...
      <v-col cols="12" md="10" lg="8">
        <v-card>
          <v-toolbar color="success" dark>
            ...
            <v-toolbar-title>Gestión de Alumnos</v-toolbar-title>
            <v-spacer></v-spacer>
    ...
    <v-dialog v-model="dialog" max-width="500px">
      <v-card>
        <v-card-title>
          <span class="text-h5">{{ editedItem.id ? 'Editar Alumno' : 'Nuevo Alumno' }}</span>
        </v-card-title>
    ...
    <v-dialog v-model="dialogDelete" max-width="500px">
      <v-card>
        <v-card-title class="text-h5">¿Estás seguro de eliminar este alumno?</v-card-title>
...
const save = async () => {
  ...
  try {
    if (editedItem.value.id) {
      ...
      showSnackbar('Alumno actualizado');
    } else {
      await axios.post('/users', editedItem.value);
      showSnackbar('Alumno creado');
    }
    loadAlumnos();
    closeDialog();
  } catch (error) {
